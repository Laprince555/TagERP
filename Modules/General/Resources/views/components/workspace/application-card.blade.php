...
<{{ $tag }}
    ...
    @endif
    {{ $attributes->class([
        'group relative flex min-h-[12rem] flex-col overflow-hidden rounded-[1.5rem] border border-[var(--color-glass-border)] bg-[var(--color-surface-1)] p-5 shadow-lg shadow-[var(--color-card-shadow)] transition duration-300',
...
>
    ...
    <div class="flex items-start justify-between gap-3">
        <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[linear-gradient(135deg,var(--color-primary-soft),var(--color-accent-soft))] text-[var(--color-primary)] shadow-inner">
            <flux:icon :name="$application['icon']" class="h-6 w-6" />
        </div>
    ...
    <div class="mt-4 flex-1 space-y-2">
        <flux:heading size="lg" class="line-clamp-2">{{ $application['name'] }}</flux:heading>

        <p class="line-clamp-3 text-sm leading-6 text-[var(--color-soft-text)]">
            {{ $application['description'] !== '' ? $application['description'] : __('messages.workspace.no_description') }}
        </p>
    </div>

    <div class="mt-5 flex items-center justify-between gap-3 border-t border-[var(--color-glass-border)] pt-4">
        <span class="truncate font-mono text-xs font-bold uppercase tracking-[0.14em] text-[var(--color-muted-text)]">{{ $application['code'] }}</span>
