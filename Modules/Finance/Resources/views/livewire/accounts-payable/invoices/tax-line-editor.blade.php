<div class="min-h-screen">
    <section class="mx-auto max-w-5xl px-4 py-6 sm:px-6 lg:px-8">
        <div class="mb-6">
            ...
            <flux:text class="mt-1">{{ __('Tax Lines') }}</flux:text>
        ...
        <div class="space-y-3">
            @foreach ($rows as $index => $row)
                <div wire:key="tax-line-{{ $row['id'] ?? 'new-'.$index }}" class="flex items-end gap-3 rounded-lg border border-zinc-200 p-3 dark:border-zinc-700">
                    <div class="flex-1">
                        <flux:select wire:model="rows.{{ $index }}.tax_id" :label="__('Tax')">
                            <flux:select.option value="">{{ __('Select a tax') }}</flux:select.option>
                            @foreach ($this->taxOptions as $tax)
                    ...
                    <div class="w-36">
                        <flux:input type="number" step="any" wire:model="rows.{{ $index }}.taxable_amount" :label="__('Taxable Amount')" />
                    </div>
                    <div class="w-36">
                        <flux:input type="number" step="any" wire:model="rows.{{ $index }}.tax_amount" :label="__('Tax Amount')" />
                    </div>
        ...
        <div class="mt-4 flex items-center justify-between">
            <flux:button wire:click="addRow" variant="filled" icon="plus">{{ __('Add Tax Line') }}</flux:button>
            <flux:button wire:click="save" variant="primary">{{ __('Save Tax Lines') }}</flux:button>
        </div>
    </section>
</div>
