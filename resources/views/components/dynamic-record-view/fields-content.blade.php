...
<div class="rounded-lg border border-app p-4">
    @if ($content->getHeading())
        <flux:heading size="lg" class="mb-3">{{ $content->getHeading() }}</flux:heading>
    @endif
    ...
    <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        @foreach ($content->getFields() as $field)
            ...
            @if ($field instanceof \App\Support\DynamicRecordView\Core\Fields\RecordReferenceViewField)
                ...
                <div class="flex flex-col gap-1" style="grid-column: span {{ $field->getColumnSpan() }} / span {{ $field->getColumnSpan() }}" wire:key="field-{{ $field->getKey() }}">
                    <dt class="flex items-center gap-1 text-xs font-medium uppercase text-muted">
                        {{ $field->getLabel() }}
                    </dt>
                    <dd class="flex items-center gap-1 text-sm text-main">
                        ...
                        @else
                            <span class="text-muted">{{ $field->getPlaceholder() }}</span>
                        @endif
                    </dd>
                </div>
            @else
                <div class="flex flex-col gap-1" style="grid-column: span {{ $field->getColumnSpan() }} / span {{ $field->getColumnSpan() }}" wire:key="field-{{ $field->getKey() }}" @if ($field->getTooltip()) title="{{ $field->getTooltip() }}" @endif>
                    <dt class="flex items-center gap-1 text-xs font-medium uppercase text-muted">
                        @if ($field->getIcon())
                            <flux:icon :name="$field->getIcon()" class="size-3" />
                        @endif
                        {{ $field->getLabel() }}
                    </dt>
